...
export default function AppView({ initialProject, onProjectSave, clearInitialProject }: AppViewProps) {
  ...
  const initializeEditingStates = (currentProposal: ProposalOutput | null) => {
    setEditingStates({
      appName: false,
  ...
  const resetAppCreationState = (clearPromptField = false) => {
    ...
    clearInitialProject(); // Clear any loaded project from parent
  };
  ...
  useEffect(() => {
    if (initialProject) {
      ...
      // Important: Call clearInitialProject after consuming it to prevent re-processing on re-renders
      // However, this might be too soon if AppView re-renders before all state is set.
      // Consider moving this call to a point where initialProject is definitively processed.
      // For now, let's assume this useEffect handles it completely on first applicable render.
      // clearInitialProject(); // This was causing issues, let parent handle clearing or AppView manages a "processed" flag.
    } else {
      // If there's no initial project, or it's cleared, reset to default state
      // resetAppCreationState(true); // This might be too aggressive if called on every render without initialProject
    }
  }, [initialProject]); // Only re-run if initialProject changes
  ...
  const handleSaveToLibrary = () => {
    ...
    onProjectSave(projectToSave); // Call parent's save function
  ...
  const handleNextStep = () => {
    ...
    if (currentIndex < stepsConfig.length - 1) {
      ...
      if (isStepCompleted(currentStep) || (currentStep === 'ideas' && selectedIdea) || (currentStep === 'proposal' && proposal)) {
        ...
        toast({ title: "Cannot Proceed", description: `Please complete "${stepsConfig[currentIndex].title}" step.`, variant: "destructive" });
      }
    }
  };
